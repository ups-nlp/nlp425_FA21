...
class MonteAgent(Agent):
    ...
    def __init__(self, env: FrotzEnv, num_steps: int):
        ...
        self.reward = rewards.Additive_Reward()
    ...
    def take_action(self, env: FrotzEnv, history: list) -> str:
        ...
        while((seconds_elapsed < time_limit or count <= minimum)):
            ...
            new_node = mcts_agent.tree_policy(
                self.root, env, self.explore_const, self.reward)
            # Determine the simulated value of the new node
            delta = mcts_agent.default_policy(
                new_node, env, self.simulation_length, self.reward)
        ...
        if config.VERBOSITY > 0:
            ...
            for child in self.root.children:
                print(child.get_prev_action(), ", count:", child.visited, ", value:", child.sim_value,
                      "normalized value:", self.reward.select_action(env, child.sim_value, child.visited, None))

        # Pick the next action
        self.root, score_dif = mcts_agent.best_child(
            self.root, self.explore_const, env, self.reward, True)
